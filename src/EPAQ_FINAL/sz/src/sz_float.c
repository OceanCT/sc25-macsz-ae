...
unsigned char * SZ_compress_float_2D_MDQ_nonblocked_with_blocked_regression(float *oriData, size_t r1, size_t r2, float realPrecision, size_t * comp_size){
	...
	// unsigned int treeByteSize = num_elements * 4 * 2;
	...
	unsigned char * result = (unsigned char *) calloc(meta_data_offset + exe_params->SZ_SIZE_TYPE + sizeof(float) + sizeof(int) + sizeof(int) + treeByteSize * 5 + 3*num_blocks*sizeof(int) + num_blocks * sizeof(unsigned short) + num_blocks * sizeof(unsigned short) + num_blocks * sizeof(float) + total_unpred * sizeof(float) + num_elements * sizeof(int), 1);
	...
	size_t typeArray_size = 0;
	encode_withEPAQ(result_type, num_elements, result_pos, &typeArray_size);
	printf("typeArray compressed size: %ld\n", typeArray_size);
...
unsigned char * SZ_compress_float_3D_MDQ_random_access_with_blocked_regression(float *oriData, size_t r1, size_t r2, size_t r3, double realPrecision, size_t * comp_size){
	...
	result_pos += total_unpred * sizeof(float);
	size_t typeArray_size = 0;
	encode_withEPAQ(result_type, num_blocks*max_num_block_elements, result_pos, &typeArray_size);
	printf("typeArray compressed size: %ld\n", typeArray_size);
	result_pos += typeArray_size;
	size_t totalEncodeSize = result_pos - result;
	free(indicator);
...
unsigned char * SZ_compress_float_1D_MDQ_decompression_random_access_with_blocked_regression(float *oriData, size_t r1, double realPrecision, size_t * comp_size){
	...
	for(size_t i=0; i<num_x; i++){
		size_t typeArray_size = 0;
		encode_withEPAQ(type, max_num_block_elements, type_array_buffer_pos, &typeArray_size);
