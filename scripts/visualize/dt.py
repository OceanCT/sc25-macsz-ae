...
for ax, dataset in zip(axes, list(dataset_map.values())[:4]):  # Limit to 4 datasets
    for i, (error_bound, color, hatch) in enumerate(zip(error_bounds, colors, hatches)):
        ...
        avg_compression_time = dataset_df.groupby("Method")["Decompressiontime"].mean()        # Plot bars with offset positions
    ...
    # ax.set_xlabel("Compression Method", fontsize=14, fontweight='bold')
        ...
        # cr_improve = avg_compression_time['mac'] / avg_compression_time['orisz']
        # print(dataset, cr_improve)
            print(dataset, error_bound, avg_compression_time)
...
# Save and display
plt.savefig("./pics/dtime.pdf", bbox_inches='tight')
plt.show()
