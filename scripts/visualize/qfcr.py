...
desired_order = ['orisz', 'adt_fse', 'mac', 'lpaq']
df = pd.read_csv('./datap/pw_rel.csv')
table_rows = []
# print(df)
for dataset in list(dataset_map.values())[:4]:
    for error_bound in error_bounds:
        dataset_df = df[(df["Precision"] == error_bound) & (df["Dataset"] == dataset) & (df["Method"] != "zfp")]
        dataset_df["CompressedQfsize"] = dataset_df["CompressedQfsize"].astype(int)
        dataset_df["Qfsize"] = dataset_df["Qfsize"].astype(int)
        avg_compressed_sizes = dataset_df.groupby("Method")["CompressedQfsize"].mean()
        avg_ori_size = dataset_df.groupby("Method")["Qfsize"].mean()
        avg_compression_ratio = 4 * avg_ori_size / avg_compressed_sizes
